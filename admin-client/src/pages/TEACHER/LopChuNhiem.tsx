...
export default function LopChuNhiem() {
  ...
  const onEdit = (s: Student) => {
    ...
    form.setFieldsValue({
      ...s,
      Ngaysinh: s.Ngaysinh ? dayjs(s.Ngaysinh) : undefined,
    });
    setOpen(true);
  };
  const onDelete = async (id: string) => {
    await api.delete(`/teacher/me/homerooms/${malop}/students/${id}`);
  ...
  const submit = async () => {
    ...
    try {
      if (editingId) {
        await api.patch(
          `/teacher/me/homerooms/${malop}/students/${editingId}`,
          payload
        );
